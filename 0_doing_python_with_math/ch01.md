...
you to type something, as shown at [➋], and press [ENTER]{.small}.
...
To get around all this, we will use the [is\_integer()] method
...
Here, we call the method [is\_integer()] to check if 1.1 is an
...
We can use [is\_integer()] to filter out noninteger input
...
[]{#page_12}Similarly, the [complex()] function can convert a
...
factor. We'll use the []{#page_13}[range()] function to write
...
the [is\_integer()] method [➋] to make sure the input is
...
```python
[]{#page_16}Now that we've seen how [format()] works, we're
...
The function [multi\_table()] implements the main
...
[{0:.2f}], []{#page_17}meaning that we want only two numbers
...
[ENTER]{.small}), the [while] loop continues
