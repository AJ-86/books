...
like "Is 4 a factor of 1024?", you can use the [is_factor()]
function:
...
conversions for us. This program will present a menu to
...
   '''
   ...
   def print_menu():  # Function to print the menu
       print('1. Kilometers to Miles')
       print('2. Miles to Kilometers')

   def km_miles():  # Function to convert kilometers to miles
   ...
   def miles_km():  # Function to convert miles to kilometers
   ...
   if __name__ == '__main__':
       ...
       if choice == '1':
           km_miles()  # If choice is 1, call km_miles()

       if choice == '2':
           miles_km()  # If choice is 2, call miles_km()
```

This is a slightly longer program than the others, but not to worry.
It's actually simple. Let's start from [➊]. The
[print_menu()] function is called, which prints a menu with
...
miles), the function [km_miles()] is called. If the choice is
entered as 2 (miles to kilometers), the function [miles_km()]
...
[km_miles()] and miles for [miles_km()]). The
...
The user is asked to enter a choice at [➊]. The choice
...
```
>>> [D = (b**2 -- 4*a*c)**0.5]  # Calculating the square root of b^2 - 4ac
```
...
```
>>> [x_1 = (-b + D)/(2*a)]  # Calculating x1
>>> [x_1]  # Displaying the value of x1
-1.0
>>> [x_2 = (-b - D)/(2*a)]  # Calculating x2
>>> [x_2]  # Displaying the value of x2
-1.0
```
...
    D = (b*b - 4*a*c)**0.5  # Calculating the square root of b^2 - 4ac
    x_1 = (-b + D)/(2*a)  # Calculating x1
    x_2 = (-b - D)/(2*a)  # Calculating x2

    print('x1: {0}'.format(x_1))  # Displaying the value of x1
    print('x2: {0}'.format(x_2))  # Displaying the value of x2

if __name__ == '__main__':
    a = input('Enter a: ')  # Taking input for a
    b = input('Enter b: ')  # Taking input for b
    c = input('Enter c: ')  # Taking input for c
    roots(float(a), float(b), float(c))  # Calling the roots function
```
...
Your program should use the [is_integer()] method
...
   Fraction operations
   '''
   from fractions import Fraction  # Importing Fraction from fractions module

   def add(a, b):
       print('Result of Addition: {0}'.format(a+b))  # Adding two fractions and printing the result

   if __name__ == '__main__':
       a = Fraction(input('Enter first fraction: '))  # Taking input for first fraction
       b = Fraction(input('Enter second fraction: '))  # Taking input for second fraction
       op = input('Operation to perform - Add, Subtract, Divide, Multiply: ')  # Taking input for operation
       if op == 'Add':
           add(a,b)  # If operation is 'Add', call the add function
```
...
Try adding support for other operations such as subtraction,
...
   def fun():
       print('I am in an endless loop')  # Printing a statement

   if __name__ == '__main__':
       while True:  # Infinite loop
           fun()  # Calling the fun function
           answer = input('Do you want to exit? (y) for yes ')  # Asking the user if they want to exit
           if answer == 'y':
               break  # If user input is 'y', exit the loop
```
...
execution---that is, it prints the string again and
...
    for i in range(1, 11):
        print('{0} x {1} = {2}'.format(a, i, a*i))  # Printing the multiplication table

if __name__ == '__main__':


    while True:  # Infinite loop
        a = input('Enter a number: ')  # Taking user input for a number
        multi_table(float(a))  # Calling the multi_table function with user input
